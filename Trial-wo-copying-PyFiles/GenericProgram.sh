...
for a in {001,002,005,010,020,050,100,200,500}; do
    ...
    for b in {1,2}; do #,3,4,5}; do 
        echo "        B$b"
        sed -i "s/BBB/${b}/g" ParametersABLN.txt
        #cat ParametersABLN.txt
        for l in {0,1,2}; do #,3,4,5}; do
            echo "            L$l"
            sed -i "s/LLL/${l}/g" ParametersABLN.txt
            for n in {001,002,003,004}; do #,005,006,007,008,009,010,015,020,025,030,035,040,045,050,060,070,080,090,100}; do
