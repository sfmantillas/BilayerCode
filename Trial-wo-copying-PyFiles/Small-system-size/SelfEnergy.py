...
eCharge = 50/100.#Parameters[1]
eLayers = 1#Parameters[1]
eMomAng = 1#Parameters[2]
ThetaBins = 100#int(Parameters[3]*100)
...
HartreeFock = np.zeros((2*ThetaBins))
...
for m1 in range(0,ThetaBins): 
    ...
    HartreeFock[ThetaBins-m1-1] = - ( 0*Momentum(mm1,nTan) + HartreeFock(Momentum(mm1,nTan)) )
...
for m1 in range(0,ThetaBins): 
    ...
    #ScaleFactor[m1] = +np.sqrt( nTan * np.tan( mm1*DeltaThet )**(2*nTan-1) ) / np.cos( mm1*DeltaThet ) / eLength
    #m1Indices[ThetaBins-m1-1] = -m1-1
    HartreeFock[ThetaBins+m1] = ( 0*Momentum(mm1,nTan) + HartreeFock(Momentum(mm1,nTan)) )
    SelfEnergy[ThetaBins+m1] = ( Momentum(mm1,nTan) + HartreeFock(Momentum(mm1,nTan)) )


plt.clf()
plt.figure(figsize=(10,10))
#plt.semilogx(MomentumAxis,SelfEnergy)
#plt.plot(MomentumAxis,SelfEnergy)
plt.loglog(MomentumAxis,SelfEnergy)
plt.plot(MomentumAxis,HartreeFock)
plt.plot(MomentumAxis,MomentumAxis)
plt.xlim(1e-15,1e+1)
plt.ylim(1e-15,1e+1)
#plt.xlim(-10,+10)
#plt.ylim(-10,+10)
plt.savefig("SigmaLog0050.pdf")
